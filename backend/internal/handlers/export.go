...
func (h *ExportHandler) ExportPDF(c *gin.Context) {
	...
	if !user.EffectivePlan().AtLeast(models.PlanPlus) {
		_ = c.Error(apierr.Forbidden("PDF export requires Ode+ or higher"))
		return
	}
	userID := user.ID
	...
	filename := fmt.Sprintf("ode-%s-%s.pdf", periodStr, until.Format("2006-01"))
...
func buildPDF(data *models.ExportData) *fpdf.Fpdf {
	...
	pdf.CellFormat(pageW, 14, "Ode", "", 1, "C", false, 0, "")
	...
	pdf.CellFormat(pageW, 6, "Generated by Ode · dreamlog.app", "", 1, "C", false, 0, "")
